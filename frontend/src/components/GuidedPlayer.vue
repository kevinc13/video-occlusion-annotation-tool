    ...
    currentOcclusionFlag: {
      get () {
        if (this.currentFrame) {
          let flags = this.currentFrame.user_occlusion_flags.filter(
            f => f.segmented_object_id === this.selectedObject.id)
          if (flags.length > 0) return flags[0].occluded
        }
        return -1
      },
      set (newValue) {
        let flag = this.currentFrame.user_occlusion_flags.filter(
            f => f.segmented_object_id === this.selectedObject.id)
        if (flag.length >= 1) {
